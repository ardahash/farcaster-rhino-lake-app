...
contract ProfilePicNFT is ERC1155, Ownable {
    ...
    mapping(address => uint256) public activePfpId;
    ...
    function setActive(uint256 tokenId) external {
        _requireValidToken(tokenId);
        require(balanceOf(msg.sender, tokenId) > 0, "NOT_OWNED");

        activePfpId[msg.sender] = tokenId;
    }

    function activeOf(address user) external view returns (uint256) {
        return activePfpId[user];
    }
